from dataclasses import dataclass, field
from typing import Optional, List, Set, Dict
import enum
from configs.config_base import ParamsBase, ConfigBase

class OptimizerTypes(enum.Enum, str):
    ADAM = 'adam'
    ADAMW = 'adamw'
    SGD = 'sgd'

@dataclass
class ConfigClassifierOptimizer(ParamsBase):
    lr: float = 5e-4
    min_lr: float = 1e-5
    warmup_lr: float = 1e-6
    eps: float = 1e-8 # Optimizer Epsilon
    betas: Optional[List[float]] = None # field(default_factory=lambda: [0.9, 0.999])
    clip_grad: Optional[float] = None # Clip gradient norm
    skip_weight_decay_list: Set[str] = field(default_factory=set())
    weight_decay: float  = 0.05 # weight decay
    # """Final value of the
    #         weight decay. We use a cosine schedule for WD and using a larger decay by
    #         the end of training improves performance for ViTs."""
    weight_decay_end: Optional[float] = None
    optimizer_type: OptimizerTypes = OptimizerTypes.ADAMW
    momentum: float = 0.9 # SGD momentum
    #name of layers for extract from optimizer
    filter_layers: List[Dict[str, List[int]]] = field(default_factory=lambda: [])

    def __post_init__(self):
        if isinstance(self.optimizer_type, str):
            self.optimizer_type = OptimizerTypes(self.optimizer_type)

    def check_valid(self):
        pass