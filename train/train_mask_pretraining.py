...
def train_one_epoch(encoder_model: NeuralTransformerForMEM,
    ...
    step_loader = 0
    # Iterates data loaders; computes loss; updates metrics; logs results
    for data_loader, ch_names in zip(data_loader_list, ch_names_list):
        ...
        for step, (samples) in enumerate(
            ...
            my_context = encoder_model.no_sync if args.distributed and (
                    step + 1) % args.gradient_accumulation_steps != 0 else nullcontext
            with my_context():
                with torch.cuda.amp.autocast():  # enabled=False
                    outputs = encoder_model(samples, input_chans, bool_masked_pos=bool_masked_pos)
            ...
            if not math.isfinite(loss_value):
                print(f"Loss is {loss_value}, stopping training at rank {dist_utils.get_rank()}", force=True)
            ...
            grad_norm = loss_scaler(loss, optimizer, clip_grad=max_norm,
                                    parameters=encoder_model.parameters(), create_graph=is_second_order,
...
def random_masking(x: Tensor, mask_ratio: float) -> Tensor:
    """
